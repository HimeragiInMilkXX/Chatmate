    ...
    <UCard
      ...
      <template #header>
        <div class="flex items-center gap-2">
          <h2 class="text-4xl font-bold"> All Notification({{ notifications?.length }}) </h2>
        </div>
      </template>
      <template #default>
        ...
        <section class="flex flex-col pt-2 h-[50vh] overflow-y-scroll">
          ...
          <NotificationItemBig v-for="data in notifications" :key="data.id" :data="data" @update="async () => notifications = (await useNotifications().getNotifications()).data"/>
